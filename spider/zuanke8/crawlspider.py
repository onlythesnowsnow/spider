...
def get_all(spiderUrl, grade):
    ...
    for k in range(int(start_page_number), int(end_page_number)):
        viewHref = spiderUrl + str(k) + '-1-1.html'
...
def get_page(viewHref, grade, number):
    ...
    try:
        response0 = requests.get(viewHref, timeout=50, headers = headers).content
    ...
    tree = etree.HTML(response0)
    
    # 如果该帖没有这个标签，证明这个帖子已被删除
    if not soup0.findAll(attrs={'class': 'z'}):
	#logNotExist(viewHref)
        return 2
    else:
    # 如果有 就记录下版块名称
	try:
		board = soup0.findAll(attrs={'class': 'z'})[4].contents[11].get_text()
	except:
		return 2
        print 'the page belongs to '+str(board)
    # 如果有需要点击才能看的 把该网址记录到文件 并跳过 
    if soup0.findAll(attrs={'class': 'f12  quoteTips'}):
	#logNeedClick(viewHref)

	print 'we have a need_click'
        return 4
    # 如果有需要购买才能看的 把该网址记录到文件 并跳过
    if soup0.findAll(attrs={'title': '购买'}):
	#logNeedBuy(viewHref)
	print 'we have a need_buy'
        return 5
    # posts和Post_time这个列表里含有该页所有楼层的作者/时间信息
    posts = tree.xpath("//div[@class='pi']/div[@class='authi']/a/text()")
    posts_time = tree.xpath("//div[@class='pti']/div[@class='authi']/em/text()")
    #print posts
   # print posts_time
    # posts_contents这个列表里含有该页所有楼层的正文信息
    posts_contents = soup0.findAll(attrs={'class':'pct'})
    # posts_head这个列表里含有该帖子的标题信息
    posts_head = soup0.findAll(attrs={'id': 'thread_subject'})
    ...
    for post, post_time, post_contents in zip(posts, posts_time, posts_contents):
        ...
        try:
            author = post
	    print author
            time = compiled_time2.search(str(post_time)).group()
	    print time
	    #print author,time
        except:
	    ...
	    ISOTIMEFORMAT = '%Y-%m-%d %X'
            spider_time = tt.strftime(ISOTIMEFORMAT, tt.localtime(tt.time()))
	    json_dict['gmtdate'] = spider_time
 ...
 #       print lines
	...
	now_page = 1
	    ...
	    next_page_href = next_page_href + '_' + str(now_page) + '/?ordertype=asc'
	    print next_page_href

            try:
                response3 = requests.get(next_page_href, cookies=cookie,  headers=headers).content
            except:
		floor = 0
                return 1
    	        # posts这个列表里含有该页所有楼层的作者/时间信息
	    soup3 = BeautifulSoup(response3, 'lxml', from_encoding="utf-8")
    	    posts3 = tree.xpath("//div[@class='pi']/div[@class='authi']/a/text()")
    	    posts3_time = tree.xpath("//div[@class='pti']/div[@class='authi']/em/text()")
    	    # posts_contents这个列表里含有该页所有楼层的正文信息
    	    posts3_contents = soup3.findAll(attrs={'class':'pct'})
